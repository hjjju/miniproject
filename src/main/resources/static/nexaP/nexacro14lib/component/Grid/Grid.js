...
if (!nexacro.Grid) {
	...
	nexacro.GridDragEventInfo = function (obj, id, dragdata, userdata, src_comp, src_refer_comp, from_comp, from_refer_comp, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow) {
		nexacro.DragEventInfo.call(this, obj, id || "ongriddrag", dragdata, userdata, src_comp, src_refer_comp, from_comp, from_refer_comp, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);
	...
	nexacro.GridClickEventInfo = function (obj, id, beforeCell, beforeCol, beforeRow, beforeSubrow, beforePvt, afterCell, afterCol, afterRow, afterSubrow, afterPvt, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY) {
		nexacro.ClickEventInfo.call(this, obj, id || "ongridclick", button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);
		this.cell = afterCell;
		this.col = afterCol;
		this.row = afterRow;
	...
	nexacro.GridMouseEventInfo = function (obj, id, cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY) {
		nexacro.MouseEventInfo.call(this, obj, id || "ongridmouse", button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);
	...
	_pGridCell.on_find_CurrentStyle_accessibility = function (pseudo) {
		var cellinfo = this._refobj;
		if (cellinfo && !this._isSubCell) {
			var grid = this._grid, datarow = grid._getDataRow(this._rowidx);

			return cellinfo._query_pseudo_accessibility(datarow, pseudo);
		}
		return null;
	};
	...
	_pGridCell._setEnable = function (enable) {
		nexacro.Component.prototype._setEnable.call(this, enable);


		if (this._subComp) {
			var enable = this._isEnable();
			this._subComp._setEnable(enable);
		}
	};

	_pGridCell.on_change_status = function (status, pseudo) {
		if (pseudo == "focused") {
			this._grid._focused_rowidx = this._rowidx;
		}
	...
	_pGridCell.on_apply_custom_setfocus = function (evt_name, self_flag) {
		...
		grid._focused_rowidx = this._rowidx;
		...
		if (this._hasEditor() && this._grid._showEditorCell) {
			if (grid.autoenter == "select") {
				bCellFocus = false;
			}
			else {
				if (this._is_Selected() && !this._grid._showEditing) {
					bCellFocus = false;
				}
			}
		}

		if (self_flag || nexacro._enableaccessibility || (!this._no_dom_focus && bCellFocus)) {
	...
	_pGridCell.on_apply_prop_tooltip = function () {
		...
		var datarow = grid._getDataRow(this._rowidx);
		var cellinfo = this._refobj;
		var tooltiptext = this.tooltiptext = cellinfo._getTooltipText(datarow);
		var tooltiptype = cellinfo._getTooltipType();

		if (control_elem) {
			control_elem.setElementToolTip(tooltiptext, tooltiptype);
		}
	};
	...
	_pGridCell._on_last_keyup = function () {
		if (this.parent) {
			var grid = this._grid;
			if (grid && !grid._showEditing && grid._is_data_enter_apply) {
	...
	_pGridCell.on_update_position = function (resize_flag, move_flag) {
		if (resize_flag && (this._adjust_width == 0 || this._adjust_height == 0)) {
			var control_elem = this._control_element;
			if (control_elem) {
				control_elem.setElementBorder(null, null);
			}
		}
	...
	_pGridCell._getCellAccessibilityLabel = function () {
		...
		if (this._subComp) {
			...
			if (displayType == "checkbox") {
				...
				var rowidx = grid._getDataRow(this._rowidx);
				var checked = grid._treeChecked[rowidx];
				this._setAccessibilityStatChecked(checked == 1 ? true : false);
	...
	_pGridCell._common_lbuttonup = function (changedtouchinfos, elem, canvasX, canvasY, from_elem) {
		...
		if (elem != from_elem) {
			...
			if (grid._selectborder_elements.length && upelem instanceof nexacro.GridCellControlElement) {
				...
				if ((nexacro.BrowserType == "Edge" || nexacro.Browser == "IE") && !is_inGridElem) {
	...
	_pGridCell._on_touch_lbuttondown = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, isTap) {
		var grid = this._grid;
		if (application.enabletouchevent == false && isTap == false && nexacro.isTouchInteraction && grid.selectscrollmode == "select") {
			grid._common_fire_user_lbuttondown(this, ctrl_key, shift_key, meta_key, canvasX, canvasY, fire_comp, refer_comp);
		}
		return nexacro.Component.prototype._on_touch_lbuttondown.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, isTap);
	};
	...
	_pGridCell._on_lbuttonup = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem) {
		if (this._common_lbuttonup(null, elem, canvasX, canvasY, from_elem)) {
			nexacro.Component.prototype._on_lbuttonup.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem);
		}

		return true;
	};

	_pGridCell._on_dragenter = function (elem, from_comp, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp) {
		if (this._grid._movingcell) {
			this._updateCursor();
		}

		return nexacro.Component.prototype._on_dragenter.call(this, elem, from_comp, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp);
	};
	...
	_pGridCell._common_mouseenter = function (from_comp) {
		...
		if (from_comp != this) {
			...
			if (this._isEnable() && (nexacro.BrowserType == "Edge" || nexacro.Browser == "IE")) {
	...
	_pGridCell._on_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp) {
		if (this._common_mouseenter(from_comp)) {
			return nexacro.Component.prototype._on_mouseenter.call(this._grid._lastmouseentercell, elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp);
		}
	};
	...
	_pGridCell._on_mouseleave = function (elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp) {
		if (this._common_mouseleave(to_comp)) {
			if (!this._grid._lastmouseentercell) {
				this._grid._lastmouseentercell = this;
			}
			return nexacro.Component.prototype._on_mouseleave.call(this._grid._lastmouseentercell, elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp);
		}
	};
	...
	_pGridCell._on_lbuttondown = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp) {
	...
	_pGridCell.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this._subComp || from_refer_comp instanceof nexacro.GridExpand) {
			var subcomp = from_refer_comp;
			while (subcomp && subcomp instanceof nexacro.Component) {
				if (subcomp instanceof nexacro.GridCell) {
					break;
				...
				if (!this._subComp || this._subComp == subcomp) {
					break;
				}
		...
		var retn = this._grid.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, true);
		...
		if (!retn) {
			...
			while (parent) {
				if (parent.on_fire_user_onlbuttondown) {
					retn = parent.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
					if (retn) {
						break;
					}
				}
	...
	_pGridCell._common_fire_lbuttonup = function (touchinfos, changedtouchinfos, button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, obj, from_refer_comp, from_elem) {
		var retn = false;
		var window = this._getWindow();
		var orgcell = this;
		...
		if (changedtouchinfos) {
			...
			var parent = this._grid.parent;
			retn = this._grid.on_fire_user_onlbuttonup(button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, orgcell, orgcell, from_elem, true);
			...
			if (!retn) {
				while (parent) {
					if (parent.on_fire_user_onlbuttonup) {
						retn = parent.on_fire_user_onlbuttonup(button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, obj, from_refer_comp, from_elem);
						if (retn) {
							break;
						}
					}
		...
		if (!alreadyclick) {
			if (target && target.row == datarow && target.cell == cellinfo._cellidx) {
				cell.on_fire_onclick(button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, obj, from_refer_comp, "treebutton", true);
			}
			else {
					...
					if ((datarow == grid._selectinfo.curdsrow) && (cell._cellidx == grid._selectinfo.curcell)) {
						if (grid._showEditing) {
							...
							if (upelem) {
								...
								if (check) {
									cell.on_fire_onclick(button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, obj, from_refer_comp, cell._subComp ? "control" : "", true);
								}
							}
						}
						else {
							if (upelem) {
								...
								if (check) {
									cell.on_fire_onclick(button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, obj, from_refer_comp, "", true);
								}
							}
						}
					}
	...
	_pGridCell.on_fire_user_ontouchend = function (touchinfos, changedtouchinfos, from_comp, from_refer_comp) {
		this._common_fire_lbuttonup(touchinfos, changedtouchinfos, "", false, false, false, false, -1, -1, -1, -1, -1, -1, from_comp, from_refer_comp, null);
		return true;
	};

	_pGridCell.on_fire_user_onlbuttonup = function (button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, obj, refer_comp, from_elem) {
		this._common_fire_lbuttonup(null, null, button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, obj, refer_comp, from_elem);
		return true;
	};
	...
	_pGridCell._on_click = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY) {
		if (!this._hideInner) {
			if (elem instanceof nexacro.ControlElement && !this._is_dblclick) {
		...
		var retn = nexacro.Component.prototype._on_click.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY);
		this._clickitem = "";
		return retn;
	};

	_pGridCell.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem, logic) {
		if (!logic) {
			var subcomp = from_refer_comp;
			while (subcomp && subcomp instanceof nexacro.Component) {
				if (subcomp instanceof nexacro.GridCell) {
		...
		if (this._band) {
			...
			if (nexacro.BrowserType == "Edge" || nexacro.Browser == "IE") {
				if (!(this._grid.onlbuttondown && this._grid.onlbuttondown.defaultprevented == true) || (this._grid.onlbuttonup && this._grid.onlbuttonup.defaultprevented == true)) {
					...
					if (!this._grid._showEditing && frame && frame._activate) {
						this._setFocus(false);
					}
				}
			}
			...
			if (this._band.id == "body") {
				this._grid.on_fire_cellclick(this, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
			}
			else if (this._band.id == "head") {
				this._grid.on_fire_headclick(this, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
			}
			else if (this._band.id == "summ") {
				this._grid.on_fire_summclick(this, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
			}
	...
	_pGridCell._on_dblclick = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY) {
		if (!this._hideInner) {
		...
		var retn = nexacro.Component.prototype._on_dblclick.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY);
		this._clickitem = "";
		return retn;
	};

	_pGridCell.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem) {
		var subcomp = from_refer_comp;
		while (subcomp && subcomp instanceof nexacro.Component) {
			if (subcomp instanceof nexacro.GridCell) {
				break;
		...
		if (this._band) {
			nexacro._fireBeforeDblclick(from_refer_comp, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);

			if (clickitem == undefined) {
				clickitem = this._clickitem;
			}
			if (this._band.id == "body") {
				return this._grid.on_fire_celldblclick(this, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
			}
			else if (this._band.id == "head") {
				return this._grid.on_fire_headdblclick(this, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
			}
			else if (this._band.id == "summ") {
				return this._grid.on_fire_summdblclick(this, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
			}
			if (!this._is_alive) {
				return;
			}
	...
	if (nexacro.OS == "iOS" && nexacro.OSVersion >= 13 && nexacro.Browser == "MobileSafari") {
		_pGridCell._on_subcomp_killfocus = function (subcomp) {
			var element = subcomp._input_element;
			if (subcomp && element) {
				var _active = nexacro._checkActiveElement(element);
				if (!_active) {
					subcomp.on_apply_custom_setfocus("focus");
				}
			}
		};
	}
	else {
		_pGridCell._on_subcomp_killfocus = nexacro._emptyFn;
	}
	...
	_pGridCell._updateDisplayer = function () {
		...
		if (this._isChangeDisplayer(displayType, datarow) == true) {
			var _async = grid._lbuttondown_proc && !this._is_setcellproperty;

			if (this._curDisplayType != "") {
				this._destroyDisplayer(_async);
			}
			this._createDisplayer(displayType);

			if (this._subComp && !this._subComp._is_created && this.subcells.length == 0) {
	...
	_pGridCell._updateAll = function (pseudo, onlycontents) {
		...
		if (control_elem) {
			...
			var cellinfo = this._refobj;
			var datarow = grid._getDataRow(this._rowidx);
			var suppress = cellinfo._getSuppress(datarow);
			...
			if ((nexacro.Browser == "Runtime" || nexacro.Browser == "IE") && suppress != 0 && pseudo === undefined
				 && this == grid._mouseovercell && onlycontents) {
				pseudo = grid._mouseovercell._pseudo;
			}
			else {
						...
						if (show) {
							...
							if (suppinfo) {
								if (suppinfo.text_proc != 0) {
									...
									if (this._text_elem) {
										this._text_elem.setElementTextVisible(true, true);
									}
									this._showInnerElement();
								}
							}
	...
	_pGridCell._changeClientmode = function (mode) {
		...
		if (control_elem) {
			...
			if (!padding) {
				padding = this.on_find_CurrentStyle_padding(this._pseudo);
			}
	...
	_pGridCell._showEditor = function (flag, focus, show) {
		...
		if (!flag) {
			...
			if (nexacro.isTouchInteraction && grid._lbuttondown_proc) {
	...
	_pGridExpand._on_bubble_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
		...
		if (call) {
			nexacro.Component.prototype._on_bubble_mouseenter.call(this, elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}

		return true;
	};

	_pGridExpand._on_bubble_mouseleave = function (elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
		...
		if (call) {
			nexacro.Component.prototype._on_bubble_mouseleave.call(this, elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}
	...
	_pGridExpand.on_fire_user_ontouchstart = function (touchinfos, changedtouchinfos, from_comp, from_refer_comp) {
		...
		if (touchinfo) {
			...
			if (grid._showEditing && grid._currentCellEditor) {
				grid._hideEditor();
			}
			grid.on_fire_onexpanddown("", false, false, false, false, touchinfo.screenX, touchinfo.screenY, touchinfo.canvasX, touchinfo.canvasY, touchinfo.clientX, touchinfo.clientY, from_comp, from_refer_comp);
		}
		return true;
	};

	_pGridExpand.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		var grid = this._grid;
		this._cellobj.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);


		if (nexacro.SupportTouch && grid._showEditing && grid._currentCellEditor) {
			grid._hideEditor();
		}
		grid.on_fire_onexpanddown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		return true;
	};
	...
	_pGridExpand.on_fire_user_ontouchend = function (touchinfos, changedtouchinfos, from_comp, from_refer_comp) {
		...
		var touchinfo = nexacro._getFirstTouchInfo(changedtouchinfos);
		if (touchinfo) {
			this._grid.on_fire_onexpandup("", false, false, false, false, touchinfo.screenX, touchinfo.screenY, touchinfo.canvasX, touchinfo.canvasY, touchinfo.clientX, touchinfo.clientY, from_comp, from_refer_comp);
			return true;
		}
	};

	_pGridExpand.on_fire_user_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
		this._cellobj.on_fire_user_onlbuttonup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		this._grid.on_fire_onexpandup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		return true;
	};

	_pGridExpand.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this._cellobj.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
	};

	_pGridExpand.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this._cellobj.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
	};
	...
	_pGridButton.on_find_CurrentStyle_accessibility = function (pseudo) {
		...
		if (cellobj) {
			...
			if (!accessibility || accessibility._value.length == 0) {
	...
	_pGridButton._find_pseudo_obj = function (styleProp, pseudo, returnType) {
		...
		if (cellobj && grid._isFakeCell(datarow) && pseudo != "disabled") {
			pseudo = "normal";
		}
	...
	_pGridButton.on_fire_user_onkeyup = function (key_code, alt_key, ctrl_key, shift_key, meta_key, from_comp, from_refer_comp) {
		var ret = nexacro.Component.prototype.on_fire_user_onkeyup.call(this, key_code, alt_key, ctrl_key, shift_key, meta_key, from_comp, from_refer_comp);
	...
	_pGridButton._on_bubble_lbuttonup = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope) {
		var call = true;
		if (bubble_scope) {
		...
		if (call) {
			nexacro.ButtonCtrl.prototype._on_bubble_lbuttonup.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope);
		}

		return true;
	};

	_pGridButton._on_bubble_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseenter();
		}
		...
		if (call) {
			nexacro.ButtonCtrl.prototype._on_bubble_mouseenter.call(this, elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}

		return true;
	};

	_pGridButton._on_bubble_mouseleave = function (elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseleave();
		}
		...
		if (call) {
			nexacro.ButtonCtrl.prototype._on_bubble_mouseleave.call(this, elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}
	...
	_pGridButton.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		nexacro.ButtonCtrl.prototype.on_fire_user_onlbuttondown.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		return true;
	};
	...
	_pGridButton.on_fire_user_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
		this._cellobj.on_fire_user_onlbuttonup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		return true;
	};

	_pGridButton.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		this._cellobj.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, "control");
		return true;
	};

	_pGridButton.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		this._cellobj.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, "control");
		return true;
	};
	...
	_pGridButton._on_click = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY) {
		if (!this._is_alive) {
			return;
		}
	...
	_pGridButton._setDataset = function (async, row, ds_retn) {
		if (ds_retn) {
			return false;
		}
	};
	...
	_pGridButton._updateAll = function (onlycontents) {
		this.on_apply_pseudo(this._pseudo, onlycontents);
		this._setProperty();


		var control_element = this.getElement();
		if (control_element && this._displaymode && this.visible && this._grid._is_use_suppress) {
			nexacro._refreshVMLContainerElement(control_element, control_element._vml_elem);
		}
	};
	...
	_pGridBar._find_pseudo_obj = function (styleProp, pseudo, returnType) {
		...
		if (cellobj) {
			...
			if (styleProp == "barcolor") {
				controlProp = "barcolor";
			}
			else if (styleProp == "padding") {
				return null;
			}
			else if (styleProp == "progressimage") {
				controlProp = "controlimage";
			}
	...
	_pGridBar.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this._cellobj.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, "control");
	};

	_pGridBar.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this._cellobj.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, "control");
	};
	...
	_pGridEdit.on_apply_custom_setfocus = function (evt_name, callback) {
		...
		if (nexacro.OS == "iOS" && nexacro.SystemType == "iphone") {
			...
			if (api && input_elem) {
				...
				if (evt_name != "lbuttondown" && this._old_selection[0] != this._old_selection[1]) {
					...
					if (nexacro.Browser == "Gecko" && !evt_name) {
				...
				if (!this.readonly && !this.password) {
			...
			ret = nexacro.EditCtrl.prototype.on_apply_custom_setfocus.call(this, evt_name);
		}
		...
		if (!this.autoselect) {
			...
			if (evt_name == "focus" && this._edit_base_api._is_apply_autoselect && nexacro.OS != "iOS") {
				this._edit_base_api._is_apply_autoselect = false;
			}
		}
	...
	if (nexacro.OS == "iOS") {
		_pGridEdit.set_value = function (v) {
			if (this._absolutelyValue && v == this.value) {
				return;
			}

			nexacro.EditCtrl.prototype.set_value.call(this, v);
		};
	}

	_pGridEdit._on_killfocus = function (obj, fromObj) {
		var ret = nexacro.EditCtrl.prototype._on_killfocus.call(this, obj, fromObj);
		if (this._cellobj && !obj && !fromObj) {
			this._cellobj._on_subcomp_killfocus(this);
		}
		return ret;
	};
	...
	_pGridEdit._on_bubble_lbuttonup = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_lbuttonup(null, elem, canvasX, canvasY, from_elem);
		}
		...
		if (call) {
			nexacro.EditCtrl.prototype._on_bubble_lbuttonup.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope);
		}

		return true;
	};

	_pGridEdit._on_bubble_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseenter();
		}
		...
		if (call) {
			nexacro.EditCtrl.prototype._on_bubble_mouseenter.call(this, elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}

		return true;
	};

	_pGridEdit._on_bubble_mouseleave = function (elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseleave();
		}
		...
		if (call) {
			nexacro.EditCtrl.prototype._on_bubble_mouseleave.call(this, elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}
		return true;
	};
	...
	_pGridEdit.on_fire_user_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
		nexacro.EditCtrl.prototype.on_fire_user_onlbuttonup.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		this._cellobj.on_fire_user_onlbuttonup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		return true;
	};
	...
	_pGridEdit.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		nexacro.EditCtrl.prototype.on_fire_user_onlbuttondown.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		return true;
	};

	_pGridEdit.on_fire_user_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, meta_key, fire_comp, refer_comp) {
		if (nexacro.BrowserType == "Edge" && keycode == nexacro.Event.KEY_ENTER) {
		...
		return nexacro.EditCtrl.prototype.on_fire_user_onkeydown.call(this, keycode, alt_key, ctrl_key, shift_key, meta_key, fire_comp, refer_comp);
	};

	_pGridEdit.on_fire_oneditclick = function (obj, caretpos, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem) {
		nexacro.EditCtrl.prototype.on_fire_oneditclick.call(this, obj, caretpos, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem);
		return true;
	};

	_pGridEdit.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this._displaymode) {
			this.setSelect(0, 0);
		}

		nexacro.EditCtrl.prototype.on_fire_ondblclick.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, "control");
		return true;
	};

	if (nexacro.Browser == "Gecko" || nexacro.Browser == "Opera") {
		_pGridEdit._on_click = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY) {
			if (!this._is_alive) {
				return;
			}
			...
			if (visible && this._isEnable() && this.enableevent) {
				...
				var clientXY = this._getClientXY(canvasX, canvasY);
				this.on_fire_oneditclick(this, caretPos, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientXY[0], clientXY[1], this, this, clickitem);
			}
		};
	}
	else {
	...
	_pGridEdit._setDataset = function (async, row, ds_retn) {
		var cellobj = this._cellobj;
		var cellinfo = this._cellinfo;
		var grid = this._grid;
		var datarow = grid._getDataRow(cellobj._rowidx);
			...
			if (api._is_composition()) {
				...
				if (nexacro.Browser == "Safari") {
					...
					if (elem._is_ios_composition_returned) {
						api._on_default_input_blur(elem, elem._input_handle);
					}
				}
			}
		}
		...
		if (cellinfo.text._bindtype == 1) {
			...
			if (ds_retn) {
				retn = cellinfo.grid._binddataset._setColumn(datarow, cellinfo.text._bindexpr, v, fail);
			}
			else {
				if (!cellinfo.grid._binddataset.oncolumnchanged._firestat) {
	...
	if (nexacro.OS == "iOS" && nexacro.OSVersion.split(".")[0] >= 13) {
		_pGridEdit._on_input_keyinput = function (elem) {
			var api = this._edit_base_api;
			if (api._stat_focus._prev_stat == "none" && api._stat_composition._prev_stat == "none") {
				if (api._is_nipon(api._stat_composition.getData()) && api._is_composition() && !elem._is_ios_composition) {
					elem.setElementValue(api._value);
					api.setElementCaretPos(api._begin_pos, api._end_pos, elem);
					return false;
				}
			}
			nexacro.EditCtrl.prototype._on_input_keyinput.call(this, elem);
		};

		_pGridEdit._on_input_compositionupdate = function (data) {
			var api = this._edit_base_api;
			if (api) {
				api._precharcode = api._charcode;
				api._stat_composition.setStatus(nexacro.EditBase.Status.CompositionUpdate, data);
			}
		};

		_pGridEdit._on_input_compositionend = function (data) {
			var elem = this._input_element;
			if (!elem._is_ios_composition) {
				var api = this._edit_base_api;
				if (api._is_composition() && !elem._is_ios_composition) {
					elem.setElementValue(api._value);
					api.setElementCaretPos(api._begin_pos, api._end_pos, elem);
					return false;
				}
			}
			nexacro.EditCtrl.prototype._on_input_compositionend.call(this, data);
		};
	}
	...
	_pGridTextArea.on_find_CurrentStyle_align = function (pseudo) {
		...
		if (cellobj) {
			...
			var align = this._find_pseudo_obj("align", pseudo, "align");
			if (!align) {
				align = cellobj.on_find_CurrentStyle_align(pseudo);
			}
	...
	_pGridTextArea.on_find_CurrentStyle_font = function (pseudo) {
		var cellobj = this._cellobj;
	...
	_pGridTextArea.on_apply_custom_setfocus = function (evt_name, callback) {
		...
		var ret = nexacro.TextAreaCtrl.prototype.on_apply_custom_setfocus.call(this, evt_name);


		if (!this.autoselect) {
			if (callback && !this._displaymode) {
				this.setCaretPos(0);
			}
		}
		else {
	...
	if (nexacro.OS == "iOS") {
		_pGridTextArea.set_value = function (v) {
			if (this._absolutelyValue && v == this.value) {
				return;
			}

			nexacro.TextAreaCtrl.prototype.set_value.call(this, v);
		};
	}

	_pGridTextArea._on_killfocus = function (obj, fromObj) {
		var ret = nexacro.TextAreaCtrl.prototype._on_killfocus.call(this, obj, fromObj);
		if (this._cellobj && !obj && !fromObj) {
			this._cellobj._on_subcomp_killfocus(this);
		}
		return ret;
	...
	_pGridTextArea._on_bubble_lbuttonup = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_lbuttonup(null, elem, canvasX, canvasY, from_elem);
		}
		...
		if (call) {
			nexacro.TextAreaCtrl.prototype._on_bubble_lbuttonup.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope);
		}

		return true;
	};

	_pGridTextArea._on_bubble_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseenter();
		}
		...
		if (call) {
			nexacro.TextAreaCtrl.prototype._on_bubble_mouseenter.call(this, elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}

		return true;
	};

	_pGridTextArea._on_bubble_mouseleave = function (elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseleave();
		}
		...
		if (call) {
			nexacro.TextAreaCtrl.prototype._on_bubble_mouseleave.call(this, elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}
	...
	_pGridTextArea.on_fire_user_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
		nexacro.TextAreaCtrl.prototype.on_fire_user_onlbuttonup.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		this._cellobj.on_fire_user_onlbuttonup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		return true;
	};
	...
	_pGridTextArea.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		nexacro.TextAreaCtrl.prototype.on_fire_user_onlbuttondown.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		return true;
	};

	_pGridTextArea.on_fire_user_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, meta_key, fire_comp, refer_comp) {
		if (nexacro.BrowserType == "Edge" && keycode == nexacro.Event.KEY_ENTER) {
		...
		return nexacro.TextAreaCtrl.prototype.on_fire_user_onkeydown.call(this, keycode, alt_key, ctrl_key, shift_key, meta_key, fire_comp, refer_comp);
	};

	_pGridTextArea.on_fire_oneditclick = function (obj, caretpos, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem) {
		nexacro.TextAreaCtrl.prototype.on_fire_oneditclick.call(this, obj, caretpos, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem);
		return true;
	};

	_pGridTextArea.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this._displaymode) {
			this.setSelect(0, 0);
		}

		nexacro.TextAreaCtrl.prototype.on_fire_ondblclick.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, "control");
		return true;
	};

	if (nexacro.Browser == "Gecko" || nexacro.Browser == "Opera") {
		_pGridTextArea._on_click = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY) {
			if (!this._is_alive) {
				return;
			}
	...
	if (nexacro.OS == "iOS" && nexacro.OSVersion.split(".")[0] >= 13) {
		_pGridTextArea._on_input_keyinput = function (elem) {
			var api = this._edit_base_api;
			if (api._stat_focus._prev_stat == "none" && api._stat_composition._prev_stat == "none") {
				if (api._is_nipon(api._stat_composition.getData()) && api._is_composition() && !elem._is_ios_composition) {
					elem.setElementValue(api._value);
			...
			nexacro.TextAreaCtrl.prototype._on_input_keyinput.call(this, elem);
		};

		_pGridTextArea._on_input_compositionupdate = function (data) {
			var api = this._edit_base_api;
			if (api) {
				api._precharcode = api._charcode;
				api._stat_composition.setStatus(nexacro.EditBase.Status.CompositionUpdate, data);
		...
		_pGridTextArea._on_input_compositionend = function (data) {
			var elem = this._input_element;
			if (!elem._is_ios_composition) {
				var api = this._edit_base_api;
				if (api._is_composition() && !elem._is_ios_composition) {
			...
			nexacro.TextAreaCtrl.prototype._on_input_compositionend.call(this, data);
		};
	}
	...
	_pGridTextArea._setDataset = function (async, row, ds_retn) {
		var cellobj = this._cellobj;
		var cellinfo = this._cellinfo;
		var grid = this._grid;
		var datarow = grid._getDataRow(cellobj._rowidx);
		...
		if (cellinfo.text._bindtype == 1 && !this.readonly) {
			...
			if (ds_retn) {
				retn = cellinfo.grid._binddataset.oncolumnchanged._firestat ? true : cellinfo.grid._binddataset._setColumn(datarow, cellinfo.text._bindexpr, v, fail);
			}
			else {
				if (!cellinfo.grid._binddataset.oncolumnchanged._firestat) {
					cellinfo.grid._binddataset._setColumn(datarow, cellinfo.text._bindexpr, v, fail);
	...
	_pGridMaskEdit.on_apply_custom_setfocus = function (evt_name, callback) {
		...
		if (nexacro.OS == "iOS" && nexacro.SystemType == "iphone") {
			...
			if (input_elem && api) {
				...
				if ((nexacro.Browser == "Chrome" || (nexacro.Browser == "Edge" && nexacro.BrowserType == "Webkit")) && evt_name == "lbuttondown") {
			...
			ret = nexacro.MaskEditCtrl.prototype.on_apply_custom_setfocus.call(this, evt_name);
		}
	...
	_pGridMaskEdit._on_killfocus = function (obj, fromObj) {
		var ret = nexacro.MaskEditCtrl.prototype._on_killfocus.call(this, obj, fromObj);
		if (this._cellobj && !obj && !fromObj) {
			this._cellobj._on_subcomp_killfocus(this);
		}
		return ret;
	...
	_pGridMaskEdit._on_bubble_lbuttonup = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_lbuttonup(null, elem, canvasX, canvasY, from_elem);
		}
		...
		if (call) {
			nexacro.MaskEditCtrl.prototype._on_bubble_lbuttonup.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope);
		}

		return true;
	};

	_pGridMaskEdit._on_bubble_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseenter();
		}
		...
		if (call) {
			nexacro.MaskEditCtrl.prototype._on_bubble_mouseenter.call(this, elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}

		return true;
	};

	_pGridMaskEdit._on_bubble_mouseleave = function (elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseleave();
		}
		...
		if (call) {
			nexacro.MaskEditCtrl.prototype._on_bubble_mouseleave.call(this, elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}
	...
	_pGridMaskEdit.on_fire_user_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
		nexacro.MaskEditCtrl.prototype.on_fire_user_onlbuttonup.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		this._cellobj.on_fire_user_onlbuttonup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		return true;
	};
	...
	_pGridMaskEdit.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		nexacro.MaskEditCtrl.prototype.on_fire_user_onlbuttondown.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		return true;
	};

	_pGridMaskEdit.on_fire_oneditclick = function (obj, caretpos, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem) {
		nexacro.MaskEditCtrl.prototype.on_fire_oneditclick.call(this, obj, caretpos, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem);
		return true;
	};

	_pGridMaskEdit.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this._displaymode) {
			this.setSelect(0, 0);
		}

		nexacro.MaskEditCtrl.prototype.on_fire_ondblclick.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, "control");
		return true;
	};

	if (nexacro.Browser == "Gecko" || nexacro.Browser == "Opera") {
		_pGridMaskEdit._on_click = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY) {
			if (!this._is_alive) {
				return;
			}
		...
		if (!this._displaymode) {
			...
			if (edit) {
					...
					if (this._is_set_value) {
						...
						if (caretPos) {
							...
							if (caretPos.begin == caretPos.end) {
								...
								if (nexacro.Browser == "Edge" && evt_name != "lbuttondown") {
				...
				if (nexacro._enableaccessibility) {
					...
					if ((!mobile && evt_name == "focus") || evt_name == "tabkey" || evt_name == "shifttabkey" || (!mobile && !evt_name)) {
						edit.on_apply_custom_setfocus(evt_name);
					}
				}
			...
			if (this.autoselect && evt_name == "focus" && this._edit_base_api._is_apply_autoselect && nexacro.OS != "iOS") {
				this._edit_base_api._is_apply_autoselect = false;
			}
			return ret;
		}
	...
	_pGridCalendar._createDropbutton = function () {
		nexacro.CalendarCtrl.prototype._createDropbutton.call(this);
		if (this._displaymode) {
			var mobile = (nexacro.isTouchInteraction && nexacro.SupportTouch) || (nexacro._isMobile && nexacro._isMobile());
			if (mobile) {
				this.dropbutton.on_apply_custom_setfocus = function () {
				};
			}
		}
	};

	_pGridCalendar._createSpinbutton = function () {
		nexacro.CalendarCtrl.prototype._createSpinbutton.call(this);
		if (this._displaymode) {
			var mobile = (nexacro.isTouchInteraction && nexacro.SupportTouch) || (nexacro._isMobile && nexacro._isMobile());
			if (mobile) {
				this.spinupbutton.on_apply_custom_setfocus = function () {
				};
				this.spindownbutton.on_apply_custom_setfocus = function () {
				};
			}
		}
	};
	...
	_pGridCalendar._on_bubble_lbuttonup = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_lbuttonup(null, elem, canvasX, canvasY, from_elem);
		}
		...
		if (call) {
			nexacro.CalendarCtrl.prototype._on_bubble_lbuttonup.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope);
		}

		return true;
	};

	_pGridCalendar._on_bubble_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseenter();
		}
		...
		if (call) {
			nexacro.CalendarCtrl.prototype._on_bubble_mouseenter.call(this, elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}

		return true;
	};

	_pGridCalendar._on_bubble_mouseleave = function (elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseleave();
		}
		...
		if (call) {
			nexacro.CalendarCtrl.prototype._on_bubble_mouseleave.call(this, elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}
	...
	_pGridCalendar.on_notify_mobile_onspindown = function (obj, e) {
		if (this._displaymode) {
			var clickitem = "";
			...
			return this._cellobj.on_fire_onclick(e.button, e.altkey, e.ctrlkey, e.shiftkey, e.metakey, e.screenX, e.screenY, canvasX, canvasY, clientXY[0], clientXY[1], e.fromobject, e.fromreferenceobject, clickitem);
		}
		else {
			return nexacro.CalendarCtrl.prototype.on_notify_mobile_onspindown.call(this, obj, e);
		}
	};

	_pGridCalendar.on_notify_mobile_onspinup = function (obj, e) {
		if (this._displaymode) {
			var clickitem = "";
			var padding = this._cellobj.currentstyle.padding;
			var canvasX = e.canvasX + (padding ? padding.left : 0);
			...
			return nexacro.CalendarCtrl.prototype.on_notify_mobile_onspinup.call(this, obj, e);
		}
	};
	...
	_pGridCalendar.on_fire_user_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
		...
		nexacro.CalendarCtrl.prototype.on_fire_user_onlbuttonup.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		this._cellobj.on_fire_user_onlbuttonup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		return true;
	};
	...
	_pGridCalendar.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (from_refer_comp != this) {
			canvasX -= from_refer_comp._adjust_left;
			clientX -= from_refer_comp._adjust_left;
			canvasY -= from_refer_comp._adjust_top;
		...
		nexacro.CalendarCtrl.prototype.on_fire_user_onlbuttondown.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		return true;
	};

	_pGridCalendar.on_fire_sys_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, meta_key, fire_comp, refer_comp) {
		var ret = nexacro.CalendarCtrl.prototype.on_fire_sys_onkeydown.call(this, keycode, alt_key, ctrl_key, shift_key, meta_key, fire_comp, refer_comp);
	...
	_pGridCalendar.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem) {
		nexacro.CalendarCtrl.prototype.on_fire_onclick.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, this, from_refer_comp, "control");
		return true;
	};

	_pGridCalendar.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this._displaymode) {
			this.setSelect(0, 0);
		}

		nexacro.CalendarCtrl.prototype.on_fire_ondblclick.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, this, from_refer_comp, "control");
		return true;
	};
	...
	_pGridCalendar._on_click = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, meta_key, canvasX, canvasY, screenX, screenY) {
		if (!this._is_alive) {
			return;
		}
		...
		if (visible && this._isEnable() && this.enableevent) {
			var clientXY = this._getClientXY(canvasX, canvasY);
			this.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientXY[0], clientXY[1], this, this, "control");
		}
	};
	...
	_pGridCalendar._setProperty = function () {
		...
		if (this._displaymode == true) {
			var v = org_v = cellinfo._getTextValueForDisp(datarow);
			...
			var v = cellinfo._getAttrValue(cellinfo.editautoselect, datarow);
			if (v != null) {
				this.set_autoselect(v);
			}

			v = org_v = cellinfo._getValue(datarow);
		}
		...
		if (calendardisplaynulltype != "default" && (nexacro._isNull(org_v) || org_v === "")) {
			if (this._displaymode) {
				if (calendardisplaynulltype == "nullmask") {
					...
					var calendardisplaynulltext = "";
					if (calendardisplaynulltype == "nulltext") {
						calendardisplaynulltext = cellinfo._getAttrValue(cellinfo.calendardisplaynulltext, datarow);
					}
	...
	_pGridCombo.on_create_contents = function () {
		nexacro.ComboCtrl.prototype.on_create_contents.call(this);
		if (this._displaymode) {
			var mobile = (nexacro.isTouchInteraction && nexacro.SupportTouch) || (nexacro._isMobile && nexacro._isMobile());
			if (mobile) {
				this.dropbutton.on_apply_custom_setfocus = function () {
	...
	_pGridCombo.on_apply_custom_setfocus = function (evt_name) {
		if (!this._displaymode) {
			var ret;
	...
	_pGridCombo.on_apply_text = function () {
		if (this.getElement()) {
			var fds = this._filtereddataset ? this._filtereddataset : this._getFilteredDataset();
			var ds = (this.type == "filter" || this.type == "filterlike") && ((this._innerdataset && !this._innerdataset.filterstr) || !this._displaymode) ? fds : this._innerdataset;
			...
			if (ds && this._is_created) {
				if (ds.filterstr && this._displaymode) {
					var currentView = ds._viewRecords;
					ds._viewRecords = ds._rawRecords;
				}
	...
	_pGridCombo._on_bubble_lbuttonup = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_lbuttonup(null, elem, canvasX, canvasY, from_elem);
		}
		...
		if (call) {
			nexacro.ComboCtrl.prototype._on_bubble_lbuttonup.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem, bubble_scope);
		}

		return true;
	};

	_pGridCombo._on_bubble_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseenter();
		}
		...
		if (call) {
			nexacro.ComboCtrl.prototype._on_bubble_mouseenter.call(this, elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}

		return true;
	};

	_pGridCombo._on_bubble_mouseleave = function (elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseleave();
		}
		...
		if (call) {
			nexacro.ComboCtrl.prototype._on_bubble_mouseleave.call(this, elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}
	...
	_pGridCombo.on_fire_user_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, meta_key, fire_comp, refer_comp) {
		var grid = this._grid;
		...
		nexacro.ComboCtrl.prototype.on_fire_user_onkeydown.call(this, keycode, alt_key, ctrl_key, shift_key, meta_key, fire_comp, refer_comp);

		if (!this._displaymode) {
			return this._cellobj.on_fire_user_onkeydown(keycode, alt_key, ctrl_key, shift_key, meta_key, fire_comp, refer_comp);
		}
	};

	_pGridCombo.on_fire_sys_onmousewheel = function (wheelDeltaX, wheelDeltaY, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, comp, refer_comp) {
		if (!this._displaymode) {
			return nexacro.ComboCtrl.prototype.on_fire_sys_onmousewheel.call(this, wheelDeltaX, wheelDeltaY, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, comp, refer_comp);
		}
		;
	};
	...
	_pGridCombo.on_fire_user_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
		if (from_refer_comp != this) {
			canvasX -= from_refer_comp._adjust_left;
			clientX -= from_refer_comp._adjust_left;
			canvasY -= from_refer_comp._adjust_top;
		...
		nexacro.ComboCtrl.prototype.on_fire_user_onlbuttonup.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		this._cellobj.on_fire_user_onlbuttonup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		return true;
	};
	...
	_pGridCombo.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (from_refer_comp != this) {
			canvasX -= from_refer_comp._adjust_left;
			clientX -= from_refer_comp._adjust_left;
			canvasY -= from_refer_comp._adjust_top;
		...
		nexacro.ComboCtrl.prototype.on_fire_user_onlbuttondown.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		return true;
	};

	_pGridCombo.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		nexacro.ComboCtrl.prototype.on_fire_onclick.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, this, from_refer_comp, "control");
		return true;
	};

	_pGridCombo.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this._displaymode) {
			this.setSelect(0, 0);
		}

		nexacro.ComboCtrl.prototype.on_fire_ondblclick.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		this._cellobj.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, this, from_refer_comp, "control");
		return true;
	};
	...
	_pGridCombo.on_lbuttondown_basic_action = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var retn = nexacro.Component.prototype.on_lbuttondown_basic_action.call(this, elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
	...
	_pGridCombo._on_click = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY) {
		if (!this._is_alive) {
			return;
		}
	...
	_pGridCellCheckbox.on_apply_custom_pseudo = function (pseudo) {
		nexacro.CheckBoxCtrl.prototype.on_apply_custom_pseudo.call(this, pseudo);
		...
		if (accessibility != curstyle.accessibility) {
			...
			this.on_apply_style_accessibility(accessibility);
		}
	};
	...
	_pGridCellCheckbox.on_find_CurrentStyle_accessibility = function (pseudo) {
		var cellobj = this._cellobj;
	...
	_pGridCellCheckbox._find_pseudo_obj = function (styleProp, pseudo, returnType) {
		var cellinfo = this._cellinfo;
		var grid = this._grid, cellobj = this._cellobj;
		var datarow = grid._getDataRow(this._cellobj._rowidx);

		if (cellobj && pseudo != "disabled") {
			if (this._grid._isFakeCell(datarow) || (pseudo == "focused" && cellobj._rowidx != grid._focused_rowidx)) {
				pseudo = "normal";
			}
		}
	...
	_pGridCellCheckbox.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this.parent.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, "control");
	};

	_pGridCellCheckbox.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this.parent.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, "control");
	};
	...
	_pGridCheckbox.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		this._common_fire_lbuttondown();
		return this._cellobj.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
	};
	...
	_pGridCheckbox.on_fire_user_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
		nexacro.GridCellCheckbox.prototype.on_fire_user_onlbuttonup.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		this._cellobj.on_fire_user_onlbuttonup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem);
		return true;
	};

	_pGridCheckbox.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		var grid = this._grid;
	...
	_pGridCheckbox.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		var grid = this._grid;
		if (grid) {
			var datarow = grid._getDataRow(this._cellobj._rowidx);
			if (grid._isFakeCell(datarow)) {
	...
	_pGridCheckbox._on_bubble_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseenter();
		}
		...
		if (call) {
			nexacro.GridCellCheckbox.prototype._on_bubble_mouseenter.call(this, elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}

		return true;
	};

	_pGridCheckbox._on_bubble_mouseleave = function (elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseleave();
		}
		...
		if (call) {
			nexacro.GridCellCheckbox.prototype._on_bubble_mouseleave.call(this, elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope);
		}
	...
	_pGridImage.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this.parent.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, "image");
	};
	...
	_pGridImage.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this.parent.on_fire_user_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, this, from_refer_comp);
	};
	...
	_pGridImage.on_fire_user_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
		return this.parent.on_fire_user_onlbuttonup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, this, from_refer_comp, from_elem);
	};

	_pGridImage.on_fire_user_onkeydown = function (key_code, alt_key, ctrl_key, shift_key, meta_key, from_comp, from_refer_comp) {
		return this.parent.on_fire_user_onkeydown(key_code, alt_key, ctrl_key, shift_key, meta_key, this, from_refer_comp);
	};

	_pGridImage.on_fire_user_onkeyup = function (key_code, alt_key, ctrl_key, shift_key, meta_key, from_comp, from_refer_comp) {
		return this.parent.on_fire_user_onkeyup(key_code, alt_key, ctrl_key, shift_key, meta_key, this, from_refer_comp);
	};

	_pGridImage.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this.parent.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, this, from_refer_comp, "image");
	};
	...
	_pGridImage._on_loadImg = function (imgurl, w, h) {
		if (this._call_val != null && imgurl != this._call_val) {
			return;
		}
	...
	_pGridTree.on_find_CurrentStyle_align = function (pseudo) {
		var align = this._find_pseudo_obj("align", pseudo, "align");


		if (!align || !align.valign) {
			var temp_align = this.parent.on_find_CurrentStyle_align(pseudo);
			if (align) {
				align.set_valign(temp_align.valign);
			}
			else {
				align = temp_align;
			}
	...
	_pGridTree._on_bubble_mouseenter = function (elem, from_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseenter();
		}
	...
	_pGridTree._on_bubble_mouseleave = function (elem, to_comp, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
		var call = true;
		if (bubble_scope) {
			call = this._cellobj._common_mouseleave();
		}
	...
	_pGridTree._on_lbuttondown = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp) {
		if (!this._is_alive) {
			return;
		}
	...
	_pGridTree._on_lbuttonup = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, from_elem) {
		if (!this._is_alive) {
			return;
		}
	...
	_pGridTree._on_click = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY) {
		...
		if (this.visible && this._isEnable() && this.enableevent) {
			...
			var clientXY = this._getClientXY(canvasX, canvasY);
			this.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientXY[0], clientXY[1], this, this, clickitem);
		}
	};

	_pGridTree._on_dblclick = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY) {
		if (!this._is_alive) {
			return;
		}
		...
		if (this.visible && this._isEnable() && this.enableevent) {
			...
			var clientXY = this._getClientXY(canvasX, canvasY);
			this.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientXY[0], clientXY[1], this, this, clickitem);
		}
	};
		...
		if (this._btn_elem && this._grid.treeusebutton != "noclick") {
			if (this._is_elem_area(this._btn_elem, canvasX, canvasY)) {
				if (this._isEditTypeTree()) {
					...
					if (this._clicktarget != rowstatus && (nexacro.BrowserType == "Edge" || nexacro.Browser == "IE" && nexacro.BrowserVersion > 8)) {
						grid._treetarget = {
							row : datarow, 
	...
	_pGridTree.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
	...
	_pGridTree.on_fire_user_onlbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem) {
		this._common_fire_lbuttonup();
	};
	...
	_pGridTree._on_treecheckboxclick = function (obj, e) {
		...
		if (obj == this._chk_ctrl) {
			...
			checked = grid._treeChecked[rowidx] == 1 ? true : false;
			this._setAccessibilityStatChecked(checked);
		}
	};

	_pGridTree.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem) {
		return this._cellobj.on_fire_ondblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem);
	};

	_pGridTree.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem) {
		...
		return this._cellobj.on_fire_onclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, clickitem);
	};
	...
	_pGridTree.on_apply_prop_tooltip = function () {
		...
		if (control_elem) {
			...
			var cellinfo = this._cellinfo;
			var tooltiptext = cellinfo._getTooltipText(datarow);
			var tooltiptype = cellinfo._getTooltipType();
	...
	_pGridRow._resetCellsSize = function (format, startcol) {
		...
		for (var i = 0; i < cells_len; i++) {
			...
			for (var j = 0; j < subcells_len; j++) {
				...
				if (expand_ctrl) {
					left = width - subcell._expand_width;
					top = expand_ctrl.top;
					width = subcell._expand_width;
					height = expand_ctrl.height;
	...
	_pGridMatrixManager._adjustScrollRows = function (vpos, is_updatecontents) {
		var grid = this._grid, totalcnt = grid._getGridRowCount(), rows = this._rows, firstrow, lastrow, first_rowidx = grid._getFixRowCnt(), last_rowidx = totalcnt - 1, bandHeight = grid._getBandClientSize()[1], hide_rows = [], hide_row, r, l, w, h, t, band = this._band, variable_size = grid._is_variable_bodyrowsize, hide_len = 0, dir = 0, lastPosition = grid._last_scroll_top;
	...
	_pGridBand.on_find_CurrentStyle_selectborder = function (pseudo) {
		var bandinfo = this._refobj;
		if (bandinfo) {
			return bandinfo._find_gridpseudo_obj("selectborder", pseudo, null, null, true, "border");
		}
		return null;
	};
	...
	_pGridBand.on_create_contents = function () {
		...
		if (control_elem && format) {
			...
			if (this._isBody) {
				...
				var align = this.parent.on_find_CurrentStyle_align();
				if (align) {
					text_elem.setElementAlignXY(align.halign ? align.halign : "center", align.valign ? align.valign : "middle");
				}
				else {
	...
	_pGridBand.on_change_containerRect = function (width, height) {
		...
		if (grid._is_changingRect) {
			if (grid._colautofit) {
				grid._applyAutofittype(true, undefined, this);
	...
	_pGridBand.on_apply_custom_setfocus = function (evt_name, self_flag) {
		var accessibility = nexacro._enableaccessibility && this._isAccessibilityEnable();

		if (self_flag || accessibility || this._grid._is_band_focus) {
			nexacro.Component.prototype.on_apply_custom_setfocus.call(this, evt_name);
			...
			if (accessibility) {
				...
				this._grid._is_band_focus = false;
			}
		}
	};
	...
	_pGridBand.on_apply_prop_class = function () {
		...
		this._refresh_contents(true, true);
	};
	...
	_pGridBand.on_apply_prop_tooltip = function () {
		...
		if (control_elem) {
			var tooltiptext = bandinfo._getTooltipText();
			var tooltiptype = bandinfo._getTooltipType();
			control_elem.setElementToolTip(tooltiptext, tooltiptype);
		}
	};
	...
	_pGridBand.on_fire_onclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this.parent.onnodataareaclick && this.parent.onnodataareaclick._has_handlers) {
			return this.parent.on_fire_onnodataareaclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp.parent, from_refer_comp);
		}
		return false;
	};

	_pGridBand.on_fire_ondblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this.parent.onnodataareadblclick && this.parent.onnodataareadblclick._has_handlers) {
			return this.parent.on_fire_onnodataareadblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp.parent, from_refer_comp);
		}
		return false;
	};
	...
	_pGridBand._refresh_contents = function (clearCurstyle, clear_cssclass) {
		if (clearCurstyle) {
			var rows = this._get_rows();
			...
			for (var i = 0; i < rowsLen; i++) {
				...
				for (var j = 0; j < cellsLen; j++) {
					cell = cells[j];
					if (clear_cssclass) {
						var findcss_ctrl = cell._refobj._findcss_ctrl;
						findcss_ctrl._css_finder = null;
						findcss_ctrl._ref_css_finder = null;
						findcss_ctrl = null;
					}
	...
	_pGridScrollBarCtrl._set_pixelpos = function (v, evt_kind) {
		...
		this._no_set_scrollinfo = true;
		if (this._pos != v) {
			var pos = this._scroll_reverse_convert(v);
			this.pos = pos[0];
			this._pos = (this.direction == "vert") ? v : pos[1];
			...
			if (bodyband) {
				if (this.direction == "vert") {
					...
					if (format) {
						...
						v = width <= this._max ? width : this._max;
					}
				}
			}
		}
	...
	nexacro.Grid = function (id, position, left, top, width, height, right, bottom, parent) {
		...
		this._lbuttondown_elem = null;
		this._is_user_fire_event = false;
	...
	_pGrid.on_apply_custom_pseudo = function (pseudo, bClearStyleCache) {
		var curstyle = this.currentstyle;
		if (bClearStyleCache) {
	...
	_pGrid.on_created_contents = function () {
		...
		if (body_band || head_band || summ_band) {
			...
			if (this._colautofit && body_band && body_band._client_width != this._client_width) {
				this._resizeBand();
			}
	...
	_pGrid.on_apply_custom_class = function (pseudo) {
		if (pseudo === undefined) {
			if (this.vscrollbar) {
		...
		if (this._bodyBand) {
			this._bodyBand.on_apply_prop_class();
		}
		if (this._headBand) {
			this._headBand.on_apply_prop_class();
		}
		if (this._summBand) {
			this._summBand.on_apply_prop_class();
		}
	};
	...
	_pGrid._on_deactivate = function () {
		...
		if (_win) {
			if (!_win._cur_ldown_elem) {
				this._lbuttondown_elem = null;
			}
			if (!_win._keydown_element) {
				this._keydown_elem = null;
			}
		}
	};
		...
		if (this.getElement() && rowcount == 0) {
			...
			if (body) {
				...
				if (this._text_elem) {
					...
					if (align) {
						this._text_elem.setElementAlignXY(align.halign ? align.halign : "center", align.valign ? align.valign : "middle");
					}
					else {
						this._text_elem.setElementAlignXY("center", "middle");
					}
					this._text_elem.setElementVisible(true);
					this._text_elem.setElementText(text);
					this._text_elem.setElementFont(font);
	...
	_pGrid.set_visible = function (v) {
		...
		if (this.visible != v) {
			...
			if (v && this._is_created) {
				this._refreshAll();
				if ((nexacro.Browser == "Chrome" || (nexacro.Browser == "Edge" && nexacro.BrowserType == "Webkit")) && this.vscrollbar) {
	...
	_pGrid.setRealColSize = function (nPivotIndex, nColIndex, nSize, bBandIndex) {
		if (arguments.length == 3) {
			bBandIndex = nexacro._toBoolean(nSize);
			nSize = parseInt(nColIndex, 10);
			nColIndex = parseInt(nPivotIndex, 10);
			nPivotIndex = -9;
		}
			...
			nColIndex = parseInt(nColIndex, 10);
			nSize = parseInt(nSize, 10);
			bBandIndex = (nColIndex == -1) ? true : nexacro._toBoolean(bBandIndex);
			if (bBandIndex) {
				nPivotIndex = parseInt(nPivotIndex, 10);
			}
			else {
				nPivotIndex = -9;
			}
	...
	_pGrid.setCellProperty = function (strBand, nCellIdx, strPropID, varValue) {
		...
		if (cellinfo) {
			...
			if (strPropID == "displaytype" && varValue == "tree") {
			...
			else if (strPropID == "selectcolor" && this._is_use_suppress && cellinfo.suppressalign.indexOf("over") != -1) {
				this._refreshBody();
			}
			else {
				...
				else if (strPropID == "tooltiptext" || strPropID == "tooltiptype") {
					is_onlycontents = true;
				}

				if (this._changeDisplayer && strPropID == "edittype") {
					nexacro.OnceCallbackTimer.callonce(this, function () {
						this._is_setcellproperty = true;
						this._refreshCell(strBand, nCellIdx, -1, true, is_onlycontents);
						this._is_setcellproperty = false;
					});
				}
				else {
					this._refreshCell(strBand, nCellIdx, -1, true, is_onlycontents);
				...
				if (is_onlycontents && this._exprcache) {
					this._is_use_bind_expr_style[strBand] = true;
					this._is_use_bind_expr_outerstyle[strBand] = true;
				}
	...
	_pGrid.moveToNextCell = function () {
		return this._moveToCell("next", true, false, undefined, undefined, true, true);
	};

	_pGrid.moveToPrevCell = function () {
		return this._moveToCell("prev", true, false, undefined, undefined, true, true);
	};
	...
	_pGrid.updateToDataset = function () {
		if (this._showEditing) {
			var editComp = this._currentCellEditor;
			if (editComp) {
				return editComp._setDataset(false, this._currentCellRow, true);
			}
		}
		return false;
	};
	...
	_pGrid.on_vscroll = function (obj, e) {
		...
		if (!control_elem || !this._bodyBand || e.type == "trackstart" || e.type == "tracklastover" || e.type == "trackfirstover" || e.type == "wheellastover" || e.type == "wheelfirstover") {
			return;
		}
	...
	_pGrid._isWheelScrollable = function (delta) {
		...
		var ch = this._getBandClientSize()[1];
	...
	_pGrid._on_nodataareaclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (!this._is_alive) {
			return;
		}
		if (this.enable) {
			this.on_fire_onnodataareaclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		}
		return true;
	};

	_pGrid.on_fire_onnodataareaclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this.onnodataareaclick && this.onnodataareaclick._has_handlers) {
			var evt = new nexacro.MouseEventInfo(from_comp, "onnodataareaclick", button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
			return this.onnodataareaclick._fireEvent(this, evt);
		}
		return true;
	};

	_pGrid._on_nodataareadblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (!this._is_alive) {
			return;
		}
		if (this.enable) {
			this.on_fire_onnodataareadblclick(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		}
		return true;
	};

	_pGrid.on_fire_onnodataareadblclick = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this.onnodataareadblclick && this.onnodataareadblclick._has_handlers) {
			var evt = new nexacro.MouseEventInfo(from_comp, "onnodataareadblclick", button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
			return this.onnodataareadblclick._fireEvent(this, evt);
		}
		return true;
	};

	_pGrid.on_fire_user_ondragenter = function (src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		...
		return nexacro.Component.prototype.on_fire_user_ondragenter.call(this, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
	};

	_pGrid.on_fire_user_ondragleave = function (src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this._noFireDragFlag == true || (src_comp && src_comp.selectscrollmode && (src_comp.selectscrollmode == "scroll" || (nexacro._isMobile && nexacro._isMobile() && src_comp.selectscrollmode == "default")))) {
			return;
		}
		...
		return nexacro.Component.prototype.on_fire_user_ondragleave.call(this, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
	};

	_pGrid.on_fire_user_ondragmove = function (src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		...
		if (!src_comp || !src_comp.selectscrollmode || src_comp.selectscrollmode == "select" || (nexacro._isDesktop() && src_comp.selectscrollmode == "default")) {
			if (this.ondragmove && this.ondragmove._has_handlers) {
				...
				var evt = new nexacro.GridDragEventInfo(this, "ondragmove", dragdata, userdata, src_comp, src_refer_comp, from_comp, from_refer_comp, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow);
	...
	_pGrid.on_fire_sys_ondragmove = function (src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, xdeltavalue, ydeltavalue) {
		if (!this._is_drag_sameselect) {
			if (this._isAreaSelect()) {
				return this._areaselectMove(from_refer_comp, canvasX, canvasY);
			}
			else {
				return nexacro.Component.prototype.on_fire_sys_ondragmove.call(this, src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, xdeltavalue, ydeltavalue);
			}
		}
		else {
	...
	_pGrid.on_fire_user_ondrag = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, self_refer_comp) {
		var cellobj = from_refer_comp;
		cellobj = this._findCellObj(cellobj);
		...
		if (this.ondrag && this.ondrag._has_handlers) {
			...
			evt = new nexacro.GridDragEventInfo(this, "ondrag", dragdata, userdata, this, self_refer_comp, from_comp, from_refer_comp, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow);
		}
		...
		else if (!this._isAreaSelect() && this._selectscrollmode == "scroll") {
			ret = [this._noFireDragFlag, this, self_refer_comp, dragdata, userdata];
		}
		return ret;
	};

	_pGrid.on_fire_user_ondrop = function (src_comp, src_refer_comp, dragdata, userdata, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		var cellobj = from_refer_comp;
		cellobj = this._findCellObj(cellobj);
		...
		if (this.ondrop && this.ondrop._has_handlers) {
			...
			var evt = new nexacro.GridDragEventInfo(this, "ondrop", dragdata, userdata, src_comp, src_refer_comp, from_comp, from_refer_comp, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow);
	...
	_pGrid._mouseSelection = function (cellobj, ctrl_key, shift_key, meta_key, canvasX, canvasY, from_comp, from_refer_comp, no_select) {
		var editor = this._currentCellEditor;
		var fobj = from_refer_comp;
		var bhide = true;
		var _prevRowidx = cellobj._rowidx;
		var _prevCellidx = cellobj._cellidx;
		...
		if (cellobj && cellobj._type_name == "GridCell") {
			...
			if (_prevRowidx >= 0 && (_prevRowidx != cellobj._rowidx || _prevCellidx != cellobj._cellidx)) {
				cellobj = this._getCurrentBodyCell(_prevRowidx, _prevCellidx);
			}

			var band = cellobj._band.id;
			var retn = this._on_grid_lbuttondown(cellobj, band, ctrl_key, shift_key, meta_key, no_select);
				...
				if (nexacro._toBoolean(this.readonly) == false) {
					if (retn) {
						...
						if (cell && cell._hasEditor()) {
							...
							var editor = this._currentCellEditor;
							if (editor) {
								if (cell._curEditDisplay == "display") {
									if (nexacro.Browser == "Gecko" || (nexacro.Browser == "Runtime" && editor.setCaretPos)) {
								...
								if (!this._onceTime_focus && editor.setCaretPos && !editor.autoselect) {
	...
	_pGrid._common_fire_sys_lbuttondown = function (cellobj, ctrl_key, shift_key, meta_key, canvasX, canvasY, from_comp, from_refer_comp) {
		if (!this._is_alive) {
			return;
		}
		...
		if (this.selectchangetype == "up") {
			if (this._isAreaSelect() && this._selectscrollmode == "select") {
				this._mouseSelection(cellobj, ctrl_key, shift_key, meta_key, canvasX, canvasY, from_comp, from_refer_comp);
			}
			else if (this.selecttype == "multirow") {
				this._mouseSelection(cellobj, ctrl_key, shift_key, meta_key, canvasX, canvasY, from_comp, from_refer_comp, true);
			}

			if (!this._lbuttondown_elem) {
				this._setdataobj = null;
			}
		}
		else {
	...
	_pGrid._common_fire_user_lbuttondown = function (cellobj, ctrl_key, shift_key, meta_key, canvasX, canvasY, from_comp, from_refer_comp) {
		if (!this._is_alive) {
			return;
		}
	...
	_pGrid._recalcXY = function (obj, canvasX, canvasY, need_recalcXY, from_refer_comp) {
		...
		if (obj._type_name == "GridCell") {
			if (need_recalcXY) {
				...
				if (area == "body") {
					if (obj == from_refer_comp || obj == from_refer_comp.parent
						 || from_refer_comp._controlmode || from_refer_comp.parent._controlmode) {
						real_canvasX = canvasX + this._curFormat.leftWidth;
					}
					else {
						real_canvasX = canvasX;
					}
			...
			if (user_fire) {
				...
				if (this._selectscrollmode == "select") {
					if (cellobj._band && cellobj._band.id == "body") {
						if (this._isAreaSelect()) {
							this._common_fire_sys_lbuttondown(cellobj, false, false, undefined, touchinfo.canvasX, touchinfo.canvasY, from_comp, from_refer_comp);
						}

						if (nexacro.OS != "Windows" || nexacro.Browser != "Runtime") {
							if (this.selectchangetype == "down") {
								this._mouseSelection(cellobj, false, false, undefined, touchinfo.canvasX, touchinfo.canvasY, from_comp, from_refer_comp);
							}
						}
					}
				}
	...
	_pGrid.on_lbuttondown_basic_action = function (elem, button, alt_key, ctrl_key, shift_key, meta_key, canvasX, canvasY, screenX, screenY, event_bubbles, fire_comp, refer_comp, bubble_scope) {
	...
	_pGrid.on_fire_user_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, need_recalcXY) {
		return this.on_fire_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, true, need_recalcXY);
	};

	_pGrid.on_fire_sys_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, need_recalcXY) {
		return this.on_fire_onlbuttondown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, false, need_recalcXY);
	};

	_pGrid.on_fire_onlbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, user_fire, need_recalcXY) {
		var cellobj = from_refer_comp;
		...
		var window = this._getWindow();
		if (user_fire && window && window._cur_ldown_elem) {
			this._lbuttondown_elem = window._cur_ldown_elem;
			window = null;
		}
		...
		if (this.onlbuttondown && this.onlbuttondown._has_handlers) {
			...
			var evt = new nexacro.GridMouseEventInfo(obj, "onlbuttondown", cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);

			if (user_fire) {
				this._is_user_fire_event = true;
		...
		if (nexacro.isTouchInteraction) {
			if (user_fire) {
				this._resizerStart(canvasX, canvasY, cellobj, "down", from_refer_comp);
				this._common_fire_user_lbuttondown(cellobj, ctrl_key, shift_key, meta_key, canvasX, canvasY, from_comp, from_refer_comp);
			}
			else {
				if (this._selectscrollmode == "select") {
					if (cellobj._band && cellobj._band.id == "body") {
			...
			if (user_fire) {
				...
				this._common_fire_sys_lbuttondown(cellobj, ctrl_key, shift_key, meta_key, canvasX, canvasY, from_comp, from_refer_comp);
			}
		}
		return retn;
	};

	_pGrid.on_fire_user_onrbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		var cellobj = from_refer_comp;
		...
		if (this.onrbuttondown && this.onrbuttondown._has_handlers) {
			...
			var evt = new nexacro.GridMouseEventInfo(obj, "onrbuttondown", cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);
			return this.onrbuttondown._fireUserEvent(this, evt);
		}
		return false;
	};

	_pGrid.on_fire_sys_onrbuttondown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		var ret = nexacro.Component.prototype.on_fire_sys_onrbuttondown.call(this, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp);
		...
		if (cellobj && cellobj instanceof nexacro.GridCell && this.autoenter == "select") {
			...
			if (cellobj._is_Selected()) {
				...
				this._setSelectedInfo(afterCell, afterCol, null, null, null);
				...
				if (this._showEditing) {
					this._hideEditor(false, false, true);
				}
			}
		}
		return ret;
	...
	_pGrid.on_fire_user_onmousedown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this.on_fire_onmousedown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, true);
	};

	_pGrid.on_fire_sys_onmousedown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this.on_fire_onmousedown(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, false);
	};

	_pGrid.on_fire_onmousedown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, user_fire) {
		var cellobj = from_refer_comp;
		...
		if (this.onmousedown && this.onmousedown._has_handlers) {
			...
			var evt = new nexacro.GridMouseEventInfo(obj, "onmousedown", cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);
	...
	_pGrid.on_fire_user_onlbuttonup = function (button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem, need_recalcXY) {
		return this.on_fire_onlbuttonup(button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem, true, need_recalcXY);
	};

	_pGrid.on_fire_sys_onlbuttonup = function (button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem, need_recalcXY) {
		return this.on_fire_onlbuttonup(button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem, false, need_recalcXY);
	};

	_pGrid.on_fire_onlbuttonup = function (button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, from_elem, user_fire, need_recalcXY) {
		if (this._focus_proc) {
			if (!this._showEditing) {
				...
				if (this._currentCellEditor._cellobj._rowidx == this._focus_proc.parent._rowidx && this._currentCellEditor._cellobj._cellidx == this._focus_proc.parent._cellidx) {
					if (this._currentCellEditor.autoselect) {
						if (nexacro.Browser == "Chrome" || (nexacro.Browser == "Edge" && nexacro.BrowserType == "Webkit") || nexacro.Browser == "Gecko") {
							this._currentCellEditor.on_apply_custom_setfocus("focus");
						}
		...
		if (this.onlbuttonup && this.onlbuttonup._has_handlers) {
			...
			var evt = new nexacro.GridMouseEventInfo(obj, "onlbuttonup", cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow, button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY);
			if (user_fire) {
				...
				if (this.selectchangetype == "up") {
					if (this.selecttype != "area" && this.selecttype != "multiarea") {
						if (!this._is_drag_selecting) {
							this._mouseSelection(cellobj, ctrlKey, shiftKey, metaKey, canvasX, canvasY, from_comp, from_refer_comp);
							this._endExtraTrack();
						}
					}
				}
	...
	_pGrid.on_fire_user_onrbuttonup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		var cellobj = from_refer_comp;
		...
		if (this.onrbuttonup && this.onrbuttonup._has_handlers) {
			...
			var evt = new nexacro.GridMouseEventInfo(obj, "onrbuttonup", cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);
			return this.onrbuttonup._fireUserEvent(this, evt);
		}
		return false;
	};

	_pGrid.on_fire_user_onmouseup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this.on_fire_onmouseup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, true);
	};

	_pGrid.on_fire_sys_onmouseup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		return this.on_fire_onmouseup(button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, false);
	};

	_pGrid.on_fire_onmouseup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp, user_fire) {
		var cellobj = from_refer_comp;
		...
		if (this.onmouseup && this.onmouseup._has_handlers) {
			...
			var evt = new nexacro.GridMouseEventInfo(obj, "onmouseup", cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);
			if (user_fire) {
	...
	_pGrid.on_fire_user_onmousemove = function (button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		...
		if (this.onmousemove && this.onmousemove._has_handlers) {
			...
			var evt = new nexacro.GridMouseEventInfo(obj, "onmousemove", cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow, button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY);
	...
	_pGrid.on_fire_sys_onmousemove = function (button, altKey, ctrlKey, shiftKey, metaKey, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (!this.enable) {
			return true;
		}
	...
	_pGrid._on_grid_lbuttondown = function (cellobj, band, ctrlkey, shiftkey, metakey, no_select) {
		if (!this._is_alive) {
			return;
		}
		if (!this.enable) {
		...
		if ((this._isAreaSelect() || this._isMultiSelect()) && !nexacro.isTouchInteraction && !this._is_down_act) {
	...
	_pGrid.on_fire_user_onkeyup = function (keyCode, altKey, ctrlKey, shiftKey, metaKey, obj, refer_comp) {
		if (!this.enable) {
			return true;
		}
		...
		var retn = nexacro.Component.prototype.on_fire_user_onkeyup.call(this, keyCode, altKey, ctrlKey, shiftKey, metaKey, obj, refer_comp);
					...
					if (cellinfo) {
						...
						if (expandshow == "show") {
							if (this.onkeyup && this.onkeyup.defaultprevented == true) {
								...
								this.on_fire_onexpandup("none", altKey, ctrlKey, shiftKey, metaKey, -1, -1, -1, -1, -1, -1, obj, refer_comp);
							}
						}
					}
				}
	...
	_pGrid.on_fire_sys_onkeydown = function (keycode, alt_key, ctrl_key, shift_key, meta_key, fire_comp, refer_comp) {
		if (this._is_down_act) {
			this._on_last_keyup(true);
		}
	};

	_pGrid.on_fire_user_onkeydown = function (keyCode, altKey, ctrlKey, shiftKey, metaKey, obj, refer_comp) {
		...
		var window = this._getWindow();
		if (window && window._keydown_element) {
			this._keydown_elem = window._keydown_element;
			window = null;
		}
		...
		if (keyCode != nexacro.Event.KEY_TAB) {
			this._is_user_fire_event = true;
			var ret = nexacro.Component.prototype.on_fire_user_onkeydown.call(this, keyCode, altKey, ctrlKey, shiftKey, metaKey, obj, refer_comp);
			this._is_user_fire_event = false;
		...
		if (keyCode == nexacro.Event.KEY_UP) {
			...
			if (!nexacro._enableaccessibility || nexacro._enableaccessibility && this._currentBand == "body") {
				{
					...
					var editType = this.getCurEditType();

					if (refer_comp._type_name == "GridCell") {
						var cur_editType = editType;
						...
						else if (editType != "checkbox" && cur_editType == "checkbox" && this.currentcol != refer_comp._refobj._col) {
							editType = "checkbox";
						}
					}
					...
					if (cellinfo) {
						...
						if (editType == "tree" && this.treeuseexpandkey) {
							...
							if (expandshow == "show" && altKey) {
								this._is_editor_keyaction = false;
								this.on_fire_onexpanddown("none", altKey, ctrlKey, shiftKey, metaKey, -1, -1, -1, -1, -1, -1, obj, refer_comp);
							}
						}
					}
				}
			...
			if (nexacro.BrowserType == "Edge" && this._showEditing) {
				var api = this._currentCellEditor._edit_base_api;
			...
			if (this._keydown_elem && !this._is_down_act) {
				...
				if (this._isChar(keyCode) || keyCode == 25) {
					if (!this._showEditing) {
						...
						if (nexacro.Browser == "Gecko") {
							if (bEnterDown) {
								this.on_fire_onenterdown(keyCode, altKey, ctrlKey, shiftKey, metaKey, obj, firecomp, postvalue);
							}
		...
		if (bEnterDown) {
			this.on_fire_onenterdown(keyCode, altKey, ctrlKey, shiftKey, metaKey, obj, firecomp, postvalue);
			if (this._is_data_enter_apply && this.autoenter == "select") {
				var cellobj = this._findCellObj(refer_comp);
				if (cellobj && cellobj._type_name == "GridCell") {
					if (cellobj.parentcell) {
						cellobj = cellobj.parentcell;
	...
	_pGrid.on_fire_allclick = function (obj, eventid, clickitem, beforeCell, beforeCol, beforeRow, beforeSubrow, beforePvt, afterCell, afterCol, afterRow, afterSubrow, afterPvt, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_refer_comp) {
		...
		if (click && click._has_handlers && this.enableevent) {
			var evt = new nexacro.GridClickEventInfo(obj, eventid, beforeCell, beforeCol, beforeRow, beforeSubrow, beforePvt, afterCell, afterCol, afterRow, afterSubrow, afterPvt, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);
	...
	_pGrid.on_fire_cellclick = function (cellobj, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (!this._is_alive) {
			return;
		}
		...
		if (!showEditclick) {
			this.on_fire_allclick(this, "oncellclick", clickitem, beforeCell, beforeCol, beforeRow, beforeSubrow, beforePvt, afterCell, afterCol, afterRow, afterSubrow, afterPvt, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_refer_comp);
		}
	};

	_pGrid.on_fire_headclick = function (cellobj, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		...
		this.on_fire_allclick(this, "onheadclick", clickitem, beforeCell, beforeCol, beforeRow, beforeSubrow, beforePvt, afterCell, afterCol, afterRow, afterSubrow, afterPvt, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_refer_comp);
	};

	_pGrid.on_fire_summclick = function (cellobj, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (!this.enable || !this._is_alive) {
			return true;
		}
		...
		this.on_fire_allclick(this, "onsummclick", clickitem, beforeCell, beforeCol, beforeRow, beforeSubrow, beforePvt, afterCell, afterCol, afterRow, afterSubrow, afterPvt, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_refer_comp);
	};

	_pGrid.on_fire_celldblclick = function (cellobj, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (!this.enable || !this._is_alive) {
			return true;
		}
		...
		this.on_fire_allclick(this, "oncelldblclick", clickitem, beforeCell, beforeCol, beforeRow, beforeSubrow, beforePvt, afterCell, afterCol, afterRow, afterSubrow, afterPvt, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_refer_comp);
	};

	_pGrid.on_fire_headdblclick = function (cellobj, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (!this.enable || !this._is_alive) {
			return true;
		}
		...
		this.on_fire_allclick(this, "onheaddblclick", clickitem, beforeCell, beforeCol, beforeRow, beforeSubrow, beforePvt, afterCell, afterCol, afterRow, afterSubrow, afterPvt, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_refer_comp);
	};

	_pGrid.on_fire_summdblclick = function (cellobj, clickitem, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (!this.enable || !this._is_alive) {
			return true;
		}
		...
		this.on_fire_allclick(this, "onsummdblclick", clickitem, beforeCell, beforeCol, beforeRow, beforeSubrow, beforePvt, afterCell, afterCol, afterRow, afterSubrow, afterPvt, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_refer_comp);
	};
			...
			if (e.row >= 0) {
				...
				if (displayrow >= 0) {
					...
					if (bindcells) {
						...
						for (var i = 0; i < cellsLen; i++) {
							bandinfo = cells[i].parent;
							if (bandinfo && bandinfo.bandtype == "head") {
								bhead = true;
							}
							else if (bandinfo && bandinfo.bandtype == "summary") {
								bsumm = true;
							}
							else {
								if (e.col == -1 && e.colidx == -1) {
									...
									if (this._hasTree && b_change_state) {
										...
										if (csupp > 0) {
											...
											if (bind && this._currentCellEditor && cells[i]._col == this._currentCellEditor._cellinfo._col && 
												cells[i]._row == this._currentCellEditor._cellinfo._row && e.row == this._currentCellRow && !this._is_textchanged) {
	...
	_pGrid._afterRowsetChanged = function (kind, updaterow_pos, row, bchange_rowcnt, prev_rowcnt) {
		if (this.getElement()) {
			...
			if (!this._lbuttondown_proc && !this._is_data_enter_apply) {
				this._moveCellAfterFocus();
			}
		}
	};
	...
	_pGrid._on_focus = function (self_flag, evt_name, lose_focus, refer_lose_focus, new_focus, refer_new_focus) {
		...
		if (!nexacro._enableaccessibility) {
			this._is_band_focus = false;
			if (refer_new_focus instanceof nexacro.GridBand || refer_new_focus instanceof nexacro.GridRow) {
		...
		if (evt_name == "tabkey" || evt_name == "shifttabkey") {
			...
			if (this._bodyBand && (self_flag == false || nexacro._enableaccessibility)) {
				...
				if (rows.length > 0) {
					if (rows[0]._cells && rows[0]._cells.length > 0) {
						...
						if (editcell.row !== null && this.autoenter != "select") {
							var cellobj = this._getCurrentBodyCell(editcell.row, editcell.cell);
							if (cellobj) {
								if (nexacro._enableaccessibility) {
									this._currentBand = "body";
									if (evt_name == "tabkey") {
			...
			if (nexacro._enableaccessibility) {
				...
				if (self_flag == false) {
					...
					if ((evt_name == "lbuttondown" || evt_name == "rbuttondown") && cellobj && cellobj instanceof nexacro.GridCell) {
						;
					}
	...
	_pGrid.on_apply_custom_setfocus = function (evt_name, self_flag) {
		if ((nexacro._enableaccessibility && this._isAccessibilityEnable()) || (!this._bodyBand && !this._headBand && !this._summBand) || !evt_name || evt_name == "focus" || evt_name == "tabkey" || evt_name == "shifttabkey") {
			nexacro.Component.prototype.on_apply_custom_setfocus.call(this, evt_name, self_flag);
		}
	};
	...
	_pGrid.on_fire_ontextchanged = function (obj, pretext, posttext) {
		...
		if (this.ontextchanged && this.ontextchanged._has_handlers) {
			...
			this._is_textchanged = true;
			var ret = this.ontextchanged._fireEvent(this, evt);
			this._is_textchanged = false;
			return ret;
		}
		return true;
	};
	...
	_pGrid.on_fire_onenterdown = function (keyCode, altKey, ctrlKey, shiftKey, metaKey, obj, refer_comp, postvalue) {
	...
	_pGrid.on_fire_onexpanddown = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this.readonly) {
			return;
		}
		...
		if (this.onexpanddown && this.onexpanddown._has_handlers) {
			...
			var evt = new nexacro.GridMouseEventInfo(obj, "onexpanddown", cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);
			this._is_user_fire_event = true;
			var ret = this.onexpanddown._fireEvent(this, evt);
			this._is_user_fire_event = true;
			return ret;
		}
		return;
	...
	_pGrid.on_fire_onexpandup = function (button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY, from_comp, from_refer_comp) {
		if (this.readonly) {
			return;
		}
		...
		if (this.onexpandup && this.onexpandup._has_handlers) {
			...
			var row = evtinfo.row;
			var subrow = evtinfo.subrow;
			var evt = new nexacro.GridMouseEventInfo(obj, "onexpandup", cell, col, mergecell, mergecol, mergerow, pivotindex, row, subrow, button, alt_key, ctrl_key, shift_key, meta_key, screenX, screenY, canvasX, canvasY, clientX, clientY);
	...
	_pGrid._recreate = function () {
		...
		var hpos = 0, is_hmax = false;


		if (this.hscrollbar) {
			hpos = this.hscrollbar._pos;
			is_hmax = (hpos > 0) ? hpos == this.hscrollbar._max : false;
		}
		...
		if (this.hscrollbar) {
			this.hscrollbar._set_pixelpos(0);
			this.hscrollbar._set_pixelpos(is_hmax ? this.hscrollbar._max : hpos);
		}
	...
	_pGrid._isDownActionKeyMouse = function () {
		...
		if (_win && (this._lbuttondown_elem || this._keydown_elem)) {
			if (nexacro._skipDragEventAfterMsgBox && !this._is_user_fire_event) {
				return false;
			}


			if (this._lbuttondown_elem && !_win._cur_ldown_elem) {
				this._lbuttondown_elem = null;
				return false;
			}
			if (this._keydown_elem && !_win._keydown_element) {
				this._keydown_elem = null;
				return false;
			}

			var elem = this._lbuttondown_elem || this._keydown_elem, comp = _win.findComponent(elem, 0, 0)[0], isgrid = false;
	...
	_pGrid._resizeBand = function (reset) {
		...
		if (this.autofittype != "none" && this._is_changingRect && this._curFormat && (this._summBand || this._headBand)) {
			var _width = w - this._curFormat.leftWidth - this._curFormat.rightWidth;
			this._curFormat._adjustColWidth(_width, this._autofitcol_rate, false);
			this._resetRowSizeList();
			this._resetColSizeList();
		}
		var headHeight = this._getHeadHeight();
	...
	_pGrid._setContents = function (str) {
		...
		var error = nexacro._getParserError(contentsElem);
		if (error && error.message) {
			application._onSystemError(this, "SyntaxError", error.message);
		}
		else {
						...
						for (var jj = 0; jj < suppressRowLen; jj++) {
							...
							if (cellinfo) {
								for (var k = 0; k < cellcnt; k++) {
									if (k == cellinfo._cellidx) {
										continue;
									}

									grid._compareSuppressCol(i, k, cellinfo._cellidx, suppresslevel, cdatarow);
								}
							}
						}
					}
	...
	_pGrid._compareSuppressCol = function (row, cell, curcell, supplvl, cdatarow) {
		var band = this._bodyBand;
		var cells = this._curFormat._bodycells;

		var pinfo = cells[cell];
		var cinfo = cells[curcell];
	...
	_pGrid._getBandClientSize = function (band) {
		var format = this._curFormat;
		...
		if (format) {
			if (!this._bodyBand) {
				...
				clientrect = this._getAvailableRect(band);
				width = clientrect.width - format.leftWidth - format.rightWidth;
				height = clientrect.height;
			}
			else {
	...
	_pGrid._applyAutofittype = function (redraw, property_set, band) {
		...
		var height, width;
		var bandsize = this._getBandClientSize(band);
		...
		if (this._colautofit && width >= 0) {
			var is_body = (!band || (band && band._isBody)) ? true : false;
			change = format._adjustColWidth(width, this._autofitcol_rate, is_body);
		}
	...
	_pGrid._isChangeBodyColSizeList = function (columnid, cols, row) {
		...
		for (var i = 0, cells_len = cells.length; i < cells_len; i++) {
			if (cells[i].expr._bindexpr) {
				...
				else if (columnid === "" && row >= 0 && row < this._rowcount) {
					changesize = true;
				}
			}
	...
	_pGrid._resetDisplayInfo = function (reset_bandsize) {
		...
		var bandsize = this._getBandClientSize();
		var bandHeight = bandsize[1];
		var rowcount = this._getGridRowCount();
		var format = this._curFormat;
		...
		if (this._bodyrowheight > 0) {
			if (this._is_variable_bodyrowsize == false) {
				...
				this.pagerowcount = Math.floor(bandHeight / bodyRowHeight);
				this._pagerowcnt = Math.ceil(bandHeight / bodyRowHeight);
			}
			else {
				var _vpos = (this.vscrollbar) ? this.vscrollbar._pos : 0;
				var vlimit = this._control_element.vscroll_limit;
				...
				if (this._pagerowcnt == 0 && toprowpos[1] >= bandHeight) {
				...
				if (subcells.length > 0) {
					...
					if (autosizecol == "none") {
						...
						if (displayType == "checkbox") {
							...
							if (displayType == "decoratetext") {
								text = nexacro._unDecorateString(text);
							}
							...
							if (maxbyte && colspan == 1 && !this._hasTree) {
								...
								for (var j = 0; j < lcnt; j++) {
									nbyte = function () {
										return unescape(encodeURI(lines[j])).length;
									};
	...
	_pGrid._getCellRowTextSize = function (cellinfo, rowidx, text, p_cellinfo) {
		...
		if (displayType == "decoratetext") {
			var fs_expr = /<\/?fs\s+[v]\s*=(\'|\")/g;
			var fs_match = fs_expr.exec(text);
			if (fs_match) {
				var decorate_font = new nexacro.Style_font(font._value);

				var font_size, last_idx;
				var split_arr = text.split(fs_match[0]);

				for (var i = 0; i < split_arr.length; i++) {
					last_idx = split_arr[i].indexOf("'>");
					if (last_idx > -1) {
						font_size = split_arr[i].substring(0, last_idx);

						if (font_size.length > 0) {
							font_size = parseInt(font_size);
							if (font_size > decorate_font.size) {
								decorate_font.set_size(font_size);
							}
						}
					}
				}
		...
		if (displayType == "image") {
			...
			if (usewordwrap && (this._autoSizeRowProc || this._rowSizeEx) && ((word != "none" && word != false && word != "false") || linespace)) {
				...
				width -= (bord._right_width && bord._right_width <= width) ? bord._right_width : 0;
				size = nexacro._getTextSize2(letterspace, text, font, true, width, word, undefined, undefined, usedecorate);
	...
	_pGrid._makeCssRefInfoCtrl = function (ctrl) {
		var id = "";
		if (ctrl instanceof nexacro.GridControlButton) {
			id = this.controlbutton.id;
		}
		else if (ctrl instanceof nexacro.GridControlEdit) {
			id = this.controledit.id;
		}
		else if (ctrl instanceof nexacro.GridControlMaskEdit) {
			id = this.controlmaskedit.id;
		}
		else if (ctrl instanceof nexacro.GridControlCombo) {
			id = this.controlcombo.id;
		}
		else if (ctrl instanceof nexacro.GridControlCalendar) {
			id = this.controlcalendar.id;
		}
		else if (ctrl instanceof nexacro.GridControlTextArea) {
			id = this.controltextarea.id;
		}
		else {
			id = ctrl.id;
		}
	...
	_pGrid._setColSize = function (nPivotIndex, nColIndex, nSize, bBandIndex, bRedraw, autosize, noAdjust) {
		...
		if (bBandIndex == true || nPivotIndex >= -2) {
			...
			else if (areatype == "right" && nColIndex >= 0) {
				nColIndex += leftcnt;
				nColIndex += bodycnt;
			}
		...
		if (nColIndex == -1) {
			...
			nColIndex = 0;
			if (areatype != "left") {
				nColIndex = leftcnt + (areatype == "right" ? bodycnt : 0);
			}
			if (areatype != "right") {
				_colsLen = areatype == "left" ? _colsLen - bodycnt - rightcnt : _colsLen - rightcnt;
			}

			for (var i = nColIndex; i < _colsLen; i++) {
				if (autosize) {
	...
	_pGrid._isRemainAreaScroll = function () {
		...
		var bandheight = this._getBandClientSize()[1];
		var lasttoprow = 0;
	...
	_pGrid._getDepthWidth = function (datarow, cellinfo) {
		...
		var buttonwidth = 9;
		var checkwidth = 15;
		var use_image = this.treeuseimage, use_check = this.treeusecheckbox, use_btn = this.treeusebutton, use_line = this.treeuseline;
		var gap = use_line ? 16 : 11;
		var prop;
		var pseudo = "normal";
	...
	_pGrid._moveToCell = function (type, editable, colmove, area, lastcol, showcell, userfire) {
		...
		if (cell && !nexacro._enableaccessibility) {
			cell._no_dom_focus = userfire ? false : true;
	...
	_pGrid._moveCellAfterFocus = function () {
		...
		if (this.autoenter == "select" && this._currentBand == "body") {
			if (nexacro.BrowserType == "Edge" || nexacro.Browser == "Opera" || nexacro.Browser == "IE") {
				this._onceTime_focus = true;
			}

			retn = this._showEditor(false, true);
	...
	_pGrid._createTreeStates = function (keepstate, ignoreDS, recheck_leaf) {
		...
		if (initstatus == 0 || initstatus == 1) {
			...
			if (cellinfo.treestate._bindtype == 0 || nexacro._isNull(state) || state === "") {
				states[rowcount - 1] = 2;
			}
		}
		else if (initstatus == 2 || initstatus == 3) {
	...
	_pGrid._toggleTreeState = function (rowidx, redraw, v, prop_set) {
		...
		if (redraw) {
			if (retn == 2) {
				if (this._bodyBand) {
					if (this.enableredraw && !this._after_recreate && !this._after_recreate_contents_all) {
				...
				if (this._beforeEditRowIdx != currow || this._beforeEditCellIdx != curcell) {
					if (cellobj._hasEditor()) {
						...
						if (editor) {
							...
							if (editor.setCaretPos) {
								if (!editor.autoselect) {
									if (!this._onceTime_focus) {
										editor.setCaretPos(0);
									}
								}
								else {
		...
		if (!flag) {
			...
			if (editComp) {
				...
				if (cellobj) {
					if (!noApplyDataset) {
						if (setdataobj) {
							setdataobj.succ = editComp._setDataset(true, this._hasTree ? this._currentCellRow : undefined);
						}
						else {
							editComp._setDataset(true, this._hasTree ? this._currentCellRow : undefined);
						}
						...
						if (cellobj._rowidx != this._beforeEditRowIdx || cellobj._cellidx != this._beforeEditCellIdx) {
							if (this._beforeEditRowIdx >= 0 && this._beforeEditCellIdx >= 0) {
								cellobj = this._getCurrentBodyCell(this._beforeEditRowIdx, this._beforeEditCellIdx);
							}
						}
					}
			...
			if (editComp) {
				...
				if (cellobj && cellobj._subComp) {
					cellobj._subComp.set_visible(true);
				}
			}
		}
	...
	_pGrid._createEditor_button = function (cellobj) {
		...
		var id = "controlbutton" + cellobj._rowidx;
		var cButton = new nexacro.GridControlButton(id, rect.left, rect.top, rect.width, rect.height, this);
	...
	_pGrid._createEditor_edit = function (cellobj, readonly) {
		...
		var id = "controledit" + cellobj._rowidx;
		var cEdit = new nexacro.GridControlEdit(id, rect.left, rect.top, rect.width, rect.height, this);
	...
	_pGrid._createEditor_maskedit = function (cellobj, type) {
		var datarow = this._getDataRow(cellobj._rowidx);
		var rect = cellobj._setPositionInGrid();
		var id = "controlmaskedit" + cellobj._rowidx;
		var cMaskEdit = new nexacro.GridControlMaskEdit(id, rect.left, rect.top, rect.width, rect.height, this);
	...
	_pGrid._createEditor_combo = function (cellobj) {
		var rect = cellobj._setPositionInGrid();
		var id = "controlcombo" + cellobj._rowidx;
		var cCombo = new nexacro.GridControlCombo(id, rect.left, rect.top, rect.width, rect.height, this);
	...
	_pGrid._createEditor_date = function (cellobj) {
		var datarow = this._getDataRow(cellobj._rowidx);
		var rect = cellobj._setPositionInGrid();
		var id = "controlcalendar" + cellobj._rowidx;
		var cCalendar = new nexacro.GridControlCalendar(id, rect.left, rect.top, rect.width, rect.height, this);
	...
	_pGrid._createEditor_textarea = function (cellobj, readonly) {
		...
		var id = "controltextarea" + cellobj._rowidx;
		var cTextArea = new nexacro.GridControlTextArea(id, rect.left, rect.top, rect.width, rect.height, this);
	...
	_pGrid._setSelectBorder = function (pseudo) {
		if (!this._is_created) {
			return;
		}
		...
		var band = this._bodyBand;
		if (band) {
			pseudo = pseudo ? pseudo : band._pseudo;
			var selectborder = band.on_find_CurrentStyle_selectborder(pseudo);
		}
		...
		if (this._isAreaSelect() || this._isMultiSelect()) {
			...
			for (var i = 0, area_len = selected_area.length; i < area_len; i++) {
				...
				for (var row = selected_area[i].begrow, e_row = selected_area[i].endrow; row <= e_row; row++) {
					...
					for (; col <= e_col; col++) {
						...
						if (cellobj) {
							if (cellobj._subComp && !cellobj._hideInner) {
								sub_elements.push(cellobj._subComp._control_element);
							}
							else if (cellobj._text_elem) {
								sub_elements.push(cellobj._text_elem);
							}
			...
			if (row && row._cells) {
				...
				for (var j = 0, cells_len = cells.length; j < cells_len; j++) {
					if (selected_cells[j]) {
						if (cells[j]._subComp && !cells[j]._hideInner) {
							sub_comp = cells[j]._subComp;
							if (cells[j]._curDisplayType == "tree") {
								if (sub_comp._btn_elem) {
									sub_elements.push(sub_comp._btn_elem);
								}
								if (sub_comp._chk_ctrl) {
									sub_elements.push(sub_comp._chk_ctrl._control_element);
								}
								if (sub_comp._img_elem) {
									sub_elements.push(sub_comp._img_elem);
								}
								if (sub_comp._text_elem) {
									sub_elements.push(sub_comp._text_elem);
								}
							}
							sub_elements.push(sub_comp._control_element);
						}
						else if (cells[j]._text_elem) {
							sub_elements.push(cells[j]._text_elem);
						}
	...
	_pGrid._setOverlayElementProperty = function (elem, left, top, width, height, style_cells, display_text, is_fake_merge) {
		...
		if (style_cells && style_cells.length > 0) {
			...
			var tooltiptype = cellinfo._getTooltipType();
			elem.setElementToolTip(cellobj.tooltiptext, tooltiptype);
			...
			if (left <= format.leftWidth && merge_width > (width + padding.left + border._right_width)) {
