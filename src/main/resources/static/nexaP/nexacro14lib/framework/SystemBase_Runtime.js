	...
	if (!nexacro.Init_systembase_runtime) {
		...
		_pProgressData._init = function () {
			...
			if (out_datasets && out_datasets.length) {
				...
				for (var i = 0; i < outDataCnt; i++) {
					...
					if (ds) {
						this._datasets[param.rval].push({
							_isEnable : false, 
							_target_ds : ds, 
		...
		_pProgressData._needParseForFirstCount = function () {
			for (var buff_ds in this._datasets) {
				var dataset_arr = this._datasets[buff_ds];
				if (dataset_arr) {
					for (var i = 0; i < dataset_arr.length; i++) {
						if (!dataset_arr[i]._is_loaded_firstcount) {
							return true;
						}
					}
				}
		...
		_pProgressData._parseDataset = function () {
			...
			var cur_buffer_obj;
			var cur_buffer_obj_arr = this._datasets[this._cur_dataset_id];
			if (cur_buffer_obj_arr) {
				for (i = 0; i < cur_buffer_obj_arr.length; i++) {
					cur_buffer_obj = cur_buffer_obj_arr[i];
			...
			if (sep_pos > 0) {
				...
				if (remoteId && remoteId.length) {
					var buffer_obj;
					var buffer_obj_arr = this._datasets[remoteId];
					if (buffer_obj_arr) {
						this._cur_dataset_id = remoteId;
						this._datasets_in_seq[this._datasets_in_seq.length] = remoteId;

						for (i = 0; i < buffer_obj_arr.length; i++) {
							buffer_obj = buffer_obj_arr[i];
		...
		_pProgressData._parseColInfo = function () {
			...
			if (n >= 0) {
				var buffer_obj;
				var buffer_obj_arr = this._datasets[this._cur_dataset_id];
				if (buffer_obj_arr) {
					for (var i = 0; i < buffer_obj_arr.length; i++) {
						buffer_obj = buffer_obj_arr[i];
		...
		_pProgressData._parse = function (bFinal) {
			var bLoop = true;
			var pre_parse_mode;
			var rows_buffer, buffer_obj, buffer_obj_arr, ds, lines, line_idx, val, i;
			...
			while (bLoop) {
				...
				switch (this._parse_mode) {
					...
					case 6:
						...
						if (!bLoop) {
							buffer_obj_arr = this._datasets[this._cur_dataset_id];
							if (buffer_obj_arr) {
								for (i = 0; i < buffer_obj_arr.length; i++) {
									buffer_obj = buffer_obj_arr[i];
			...
			if (bFinal) {
				for (var i = 0; i < this._datasets_in_seq.length; i++) {
					...
					buffer_obj_arr = this._datasets[val];
					if (buffer_obj_arr) {
						for (var j = 0; j < buffer_obj_arr.length; j++) {
							buffer_obj = buffer_obj_arr[j];
			...
			if (this._parse_mode == 0) {
				...
				switch (this._parse_mode) {
					...
					case 6:
						...
						if (buffer_obj_arr) {
							for (var i = 0; i < buffer_obj_arr.length; i++) {
								var buffer_obj = buffer_obj_arr[i];
								if (buffer_obj._is_loaded_firstcount) {
									if (n >= 0) {
										dataIdx = this._getNextIdx([next_buffer_idx, n + rs_len]);
										buffer_obj._row_end_idx = [next_buffer_idx, n];
										this._parse_mode = 10;
									}
									else {
										bFlag = true;
										dataIdx = this._getNextIdx([next_buffer_idx, this._data_buffer[next_buffer_idx].length - 1]);
									}
								}
							}

							this._cur_idx = dataIdx;
			...
			if (this._parse_mode == 0) {
				...
				switch (this._parse_mode) {
					...
					case 6:
						...
						var buffer_obj, dataIdx, bFlag = false;
						var buffer_obj_arr = this._datasets[this._cur_dataset_id];
						if (buffer_obj_arr) {
							for (var i = 0; i < buffer_obj_arr.length; i++) {
								buffer_obj = buffer_obj_arr[i];
